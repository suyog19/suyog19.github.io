(function () {
  ...
  function create(config) {
    ...
    let loaded = false; let loading = false; let selected = ''; let records = [];
    ...
    async function load(force) { if (!config.sessionActive() || loading || (loaded && !force)) return; loading = true; try { const courses = await config.request('/admin/training/courses', { method: 'GET' }); const courseItems = Array.isArray(courses.items) ? courses.items : []; const groups = await Promise.all(courseItems.map(async (course) => { const page = await config.request('/admin/training/cohorts?courseId=' + encodeURIComponent(course.courseId) + '&limit=100', { method: 'GET' }); return Promise.all((page.items || []).map(async (cohort) => { const data = await config.request('/admin/training/cohorts/' + encodeURIComponent(cohort.cohortId) + '/gate3-summary', { method: 'GET' }); return { course, cohort, summary: data.summary || {} }; })); })); records = groups.flat(); loaded = true; renderList(); if (selected) renderDetail(records.find((item) => item.cohort.cohortId === selected)); config.setStatus('', ''); } catch (error) { fail(error); } finally { loading = false; } }
    ...
    return { load, clear() { loaded = false; selected = ''; records = []; list.replaceChildren(); detail.replaceChildren(text('p', 'Select a cohort to review.', 'admin-empty')); } };
  }
