(function () {
  ...
  function create(config) {
    ...
    function commandForm(title, command, summary, fields, payload) {
      ...
      fields.forEach((item) => form.appendChild(field(item.label, item.name, item))); form.appendChild(field(environment === 'production' ? 'I confirm this production decision against the displayed approved versions' : 'I confirm this development-only decision against the displayed versions', 'confirmation', { type: 'checkbox' })); const button = text('button', 'Submit decision', 'btn btn-secondary'); button.type = 'submit'; form.appendChild(button);
