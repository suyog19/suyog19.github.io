(function () {
  ...
  function create(config) {
    ...
    function fail(error) { config.setStatus(config.friendlyError(error), 'error'); }
    ...
    function renderCommands(data, communications, cohorts) {
      ...
      (data.learnerChangeDecisions || []).filter((item) => item.requestType === 'TRANSFER' && ['APPROVED', 'TRANSFER_OFFERED'].includes(item.decision)).forEach((decision) => {
        ...
        const choices = (cohorts || []).filter((item) => item.cohortId !== data.enrolment.cohortId && item.lifecycle === 'OPEN').map((item) => [item.cohortId, item.label + ' · v' + item.version]);
      ...
      const captures = (data.captures || []).filter((item) => item.verificationStatus === 'VERIFIED'); const decisions = data.learnerChangeDecisions || [];
      if (captures.length) detail.appendChild(commandForm('Initiate bounded test refund', [{ label: 'Mode', name: 'mode', values: [['FULL', 'Full remaining refundable amount'], ['PARTIAL_EXCEPTION', 'Approved partial exception']] }, { label: 'Amount in minor units (backend limit applies)', name: 'amountMinorUnits', type: 'number', min: '1' }, { label: 'Capture reference', name: 'providerPaymentReference', values: captures.map((item) => [item.providerPaymentReference, item.providerPaymentReference]) }, { label: 'Reason code', name: 'reasonCode', values: [['ORGANISER_DECISION', 'Organiser decision'], ['EXCESS_CAPTURE_REFUND', 'Excess capture refund']] }, { label: 'Approved decision evidence', name: 'decisionId', values: [['', 'Not applicable (excess capture only)'], ...decisions.filter((item) => Array.isArray(item.requiredActions) && item.requiredActions.includes('SOURCE_FULL_REFUND')).map((item) => [item.decisionId, item.decision + ' · ' + item.decisionId])] }, ...baseFields], (form) => command('/admin/training/payments/' + encodeURIComponent(obligation.paymentObligationId) + '/refunds', 'refund:' + obligation.paymentObligationId, { mode: form.get('mode'), amountMinorUnits: Number(form.get('amountMinorUnits')), providerPaymentReference: form.get('providerPaymentReference'), reasonCode: form.get('reasonCode'), decisionId: form.get('reasonCode') === 'EXCESS_CAPTURE_REFUND' ? null : form.get('decisionId'), expectedObligationVersion: Number(obligation.version), ...common(form) })));
      ...
      const allocatable = captures.filter((item) => item.allocationStatus !== 'ALLOCATED');
