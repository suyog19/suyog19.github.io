  ...
  function create(config) {
    ...
    function renderCommands(data, communications, cohorts) {
      ...
      if (obligation.purpose === 'BALANCE') {
        ...
        if (!current && ['OPEN', 'OVERDUE'].includes(obligation.status)) detail.appendChild(commandForm('Create and send initial test remaining-fee request', baseFields, (form) => command('/admin/training/cohorts/' + encodeURIComponent(obligation.cohortId) + '/balance-request', 'balance-request:' + obligation.paymentObligationId, { ...exact, ...common(form) })));
