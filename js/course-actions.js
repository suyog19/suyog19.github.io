(function () {
  ...
  function updateJourneyCard(card, item) { const status = card.querySelector('[data-course-status]'); const action = card.querySelector('[data-course-action="transactional"]'); if (status) { const open = item.publicStatus === 'APPLICATIONS_OPEN'; const later = item.publicStatus === 'COMING_LATER'; status.textContent = open ? 'Applications open' : later ? 'Coming later' : 'Applications not open'; status.className = `status-chip ${open ? 'status-chip--launched' : later ? 'status-chip--pipeline' : 'status-chip--unavailable'}`; } if (action) updateLink(action, item, 'TRAINING_JOURNEY', 'PATHWAY_CARD'); }
  function updateHomeCard(card, item) { const status = card.querySelector('[data-course-status]'); const action = card.querySelector('[data-home-course-action]'); if (status) { const open = item.publicStatus === 'APPLICATIONS_OPEN'; const later = item.publicStatus === 'COMING_LATER'; status.textContent = open ? 'Applications open' : later ? 'Coming later' : 'Applications not open'; status.className = `status-chip ${open ? 'status-chip--launched' : later ? 'status-chip--pipeline' : 'status-chip--unavailable'}`; } if (action) { updateLink(action, item, 'HOMEPAGE', 'COURSE_CARD'); if (!action.hidden) { action.dataset.homeEvent = 'home_training_course_action_click'; action.dataset.courseId = item.courseId; action.dataset.courseAction = item.primaryAction; } } }
  function failClosed() { document.querySelectorAll('[data-course-action="notify"], [data-course-action="interest"]').forEach(link => { link.hidden = true; link.removeAttribute('href'); }); detailActionLinks().forEach(link => { link.hidden = true; link.removeAttribute('href'); }); journeyCards().forEach(closeJourneyCard); homeCards().forEach(closeJourneyCard); }
