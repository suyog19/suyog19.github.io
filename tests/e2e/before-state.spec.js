...
test.describe('accepted BEFORE-state rendered journeys', () => {
  ...
  test('HOME-01 HOME-02 HOME-03 HOME-04 WEEKLY-01: homepage critical CTAs reach current destinations', async ({ page }) => {
    const actions = [
      ['Explore the Framework', '/framework/'], ['Browse writing', '/writing/'],
    ...
    for (const [name, expectedPath, expectedHash] of actions) {
      await page.goto('/');
      const scope = name === 'Explore the Framework' ? page.locator('.home-opening') : page;
    ...
    await page.goto('/');
    await page.locator('[data-system-id="ai-dev-orchestrator"]').click();
