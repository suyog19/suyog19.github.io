...
test('every public page exposes the approved professional-platform navigation', () => {
  ...
  assert.equal(files.length, 84, 'update the public-route classification when routes change');
  assert.equal(publicPages.length, 73);
  assert.ok(publicPages.includes('training/events/ai-engineering-roles-2026/index.html'), 'the canonical event retains the public shell');
...
test('every existing public-shell footer exposes one restrained Support link', () => {
  ...
  assert.equal(footerPages.length, 69, 'update the footer contract when public shells change');
