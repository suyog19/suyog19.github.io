...
- Article/system body template: header section (`.article-page-header`) → body section (`.article-body-section`) → optional original note (`.article-original-note`) → article feedback (`.article-feedback`) → related reading (`.article-related`). All wrapped in `.article-reading-col` for max-width centering.
- Writing article pages set `data-feedback-target="<slug>"` on the wrapping `<article>` element. Use the stable article slug, initially matching the folder name, so feedback identity does not change if a URL is renamed later.
- Writing article pages include [js/feedback.js](js/feedback.js) after [js/script.js](js/script.js). Follow the same relative-path-by-depth rule as other scripts: two-level article pages use `../../js/feedback.js`.
